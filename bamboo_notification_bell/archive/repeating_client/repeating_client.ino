...
//char bambooServer[] = "bamboo.lbi.co.uk";  
...
void makeBambooRequest() {
 // give the Ethernet shield a second to initialize:
  delay(1000);
  Serial.println("connecting to Bamboo...");

  // if you get a connection, report back via serial:
  if (client.connect(bambooServer, 80)) {
    Serial.println("connected");
    // Make a HTTP request:
    client.println("GET /rest/api/latest/result/AZCOM-DEV.json?max-results=1 HTTP/1.1");
    client.println("Host: bamboo.lbi.co.uk");
     client.println("Authorization: Basic cm9iaW5zb3M6TWFuaGF0dGFuNQ==");
